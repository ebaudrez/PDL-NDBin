...
lives_ok { PDL::NDBin->new( axes => [ [ null, 0, 0, 1 ] ] ) } 'correct arguments: one axis';
lives_ok { PDL::NDBin->new( axes => [ [ null, 0, 0, 1 ],
				      [ null, 0, 0, 1 ] ] ) } 'correct arguments: two axes';
lives_ok { PDL::NDBin->new( axes => [ [ null, 0, 0, 1 ],
				      [ null, 0, 0, 1 ],
				      [ null, 0, 0, 1 ] ] ) } 'correct arguments: three axes';
lives_ok { PDL::NDBin->new( axes => [ [ null, 0, 0, 1 ],
				      [ null, 0, 0, 1 ],
				      [ null, 0, 0, 1 ] ],
			    loop => sub {},
			    vars => [ [ null, sub {} ] ] ) } 'correct arguments: three axes, one variable';
lives_ok { PDL::NDBin->new( axes => [ [ null, 0, 0, 1 ],
				      [ null, 0, 0, 1 ],
				      [ null, 0, 0, 1 ] ],
			    loop => sub {},
			    vars => [ [ null, sub {} ],
				      [ null, sub {} ] ] ) } 'correct arguments: three axes, two variables';
lives_ok { PDL::NDBin->new( axes => [ [ null, 0, 0, 1 ],
				      [ null, 0, 0, 1 ],
				      [ null, 0, 0, 1 ] ],
			    loop => sub {},
			    vars => [ [ null, sub {} ],
				      [ null, sub {} ],
				      [ null, sub {} ] ] ) } 'correct arguments: three axes, three variables';
dies_ok { PDL::NDBin->new() } 'no arguments';
dies_ok { PDL::NDBin->new( axes => [ [ 0 ] ] ) } 'wrong arguments: 0';
dies_ok { PDL::NDBin->new( axes => [ [ null ] ] ) } 'wrong arguments: null';
dies_ok { PDL::NDBin->new( axes => [ [ null, 0 ] ] ) } 'wrong arguments: null, 0';
dies_ok { PDL::NDBin->new( axes => [ [ null, 0, 0 ] ] ) } 'wrong arguments: null, 0, 0';
dies_ok { PDL::NDBin->new( axes => [ [ null, 0, 0, null ] ] ) } 'wrong arguments: null, 0, 0, null';
dies_ok { PDL::NDBin->new( axes => [ [ null, 0, 0, 1 ],
				     [ null ] ] ) } 'wrong arguments: null, 0, 0, 1, null';
dies_ok { PDL::NDBin->new( axes => [ [ null, 0, 0, 1 ],
				     [ null, 0 ] ] ) } 'wrong arguments: null, 0, 0, 1, null, 0';
...
$expected = long( 0,2,1 );
$got = PDL::NDBin->new( axes => [ [ $x, 1, 0, 3 ] ] )->output;
is_pdl( $got, $expected, 'example from PDL::histogram' );
$got = PDL::NDBin->new( axes => [ [ $x, 1, 0, 3 ] ],
			loop => \&PDL::NDBin::default_loop,
			vars => [ [ zeroes( long, $x->nelem ), sub { shift->nelem } ] ] )->output;
...
$expected = long( [1,1],
		  ...
		  [1,0],
		  [2,1] );
$got = PDL::NDBin->new( axes => [ [ $x, 1, 1, 2 ],
				  [ $y, 1, 1, 4 ] ] )->output;
...
$got = PDL::NDBin->new( axes => [ [ $x => (1,1,4) ] ] )->output;
...
$got = PDL::NDBin->new( axes => [ [ $x, 3, 0, 7 ] ],
			loop => \&PDL::NDBin::default_loop,
			vars => [ [ $x, sub { shift->avg } ] ] )->output;
...
$got = PDL::NDBin->new( axes => [ [ $x, 1,3,5 ] ],
			loop => \&PDL::NDBin::default_loop,
			vars => [ [ $x, sub { shift->nelem || undef } ] ] )->output;
